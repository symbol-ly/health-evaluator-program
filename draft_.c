...
HealthData analyzeData(float weight, float height, int bp_sys, int bp_dias, int bs, int chol) {
    ...
    // BMI classification
    if (data.bmi < 18.5) data.bmi_status = 0;
    else if (data.bmi < 25.0) data.bmi_status = 1;
    else if (data.bmi < 30.0) data.bmi_status = 2;
    else if (data.bmi < 35.0) data.bmi_status = 3;
    else if (data.bmi < 40.0) data.bmi_status = 4;
    else data.bmi_status = 5;

    // BP
    if (bp_sys >= 180 || bp_dias >= 120) data.bp_status = 5;
    else if (bp_sys >= 140 || bp_dias >= 90) data.bp_status = 4;
    else if (bp_sys >= 130 || bp_dias >= 80) data.bp_status = 3;
    else if (bp_sys >= 120 && bp_dias < 80) data.bp_status = 2;
    else if (bp_sys < 120 && bp_dias < 80) data.bp_status = 1;
    else data.bp_status = 0;

    // Blood Sugar
    if (bs < 80) data.bs_status = 0;
    else if (bs < 90) data.bs_status = 1;
    else if (bs < 140) data.bs_status = 2;
    else if (bs < 220) data.bs_status = 3;
    else data.bs_status = 4;

    // Cholesterol
    if (chol < 200) data.chol_status = 0;
    else if (chol < 240) data.chol_status = 1;
    else data.chol_status = 2;
    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            if (!exists) {
                ...
                FILE* fp = fopen(report_file, "w");  // optional file output
                dietAddAvoid(user.analysis, fp);
                if (fp) fclose(fp);
                printf("\nDiet recommendations generated in %s\n", report_file);
            }
        }
        else if (choice == 4) {
            if (!exists) {
                ...
                if (fp) fclose(fp);
                printf("\nExercise recommendations generated in %s\n", report_file);
            }
        }
